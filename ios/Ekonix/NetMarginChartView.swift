...
struct NetMarginChartView: View {
    ...
    var body: some View {
        ZStack {
            ...
            } else if netMarginData.isEmpty {
                VStack(spacing: 20) {
                    Spacer()
                    Image(systemName: "chart.bar.xaxis")
                        .font(.system(size: 60))
                        .foregroundStyle(.gray)
                    Text("No net margin data available")
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("Trailing Twelve Months Net Margin")
                                .font(.headline)
                                        ...
                                        ZStack(alignment: .bottom) {
                                            ...
                                            HStack(alignment: .bottom, spacing: ChartConstants.barSpacing) {
                                                ForEach(Array(displayData.enumerated()), id: \.element.id) { index, point in
                                                    let heightValue = barHeight(for: point.netMargin, in: ChartConstants.chartHeight)
                                                    let offsetValue = barOffset(for: point.netMargin, barHeight: heightValue, in: ChartConstants.chartHeight)
                                                        ...
                                                        if selectedBar == point.id {
                                                            VStack(spacing: 2) {
                                                                Text(formatDate(point.period))
                                                                    .font(.caption2)
                                                                    .fontWeight(.bold)
                                                                Text(formatDetailedValue(point.netMargin))
                                                                    .font(.caption2)
                                                                    .fontWeight(.semibold)
                                                                #if DEBUG
                                                                Text(point.period)
                                            ...
                                            if netMarginRange.min < 0 && netMarginRange.max > 0 {
                                                Rectangle()
    ...
    private func loadNetMargin() {
        isLoading = true

        Task {
            do {
                let data = try await apiService.fetchTTMNetMargin(symbol: symbol)
                await MainActor.run {
                    netMarginData = data
                    isLoading = false
                    onDataLoaded?(data)

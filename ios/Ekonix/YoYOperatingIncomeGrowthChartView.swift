//
//  YoYOperatingIncomeGrowthChartView.swift
//  Ekonix
//
//  Created for operating income YoY growth chart
//

import SwiftUI

struct YoYOperatingIncomeGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let priorOperatingIncome: Double
        let shouldRender: Bool
        var id: String { period }
    }

    var growthData: [GrowthDataPoint] {
        guard operatingIncomeData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = operatingIncomeData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.operatingIncome > 0
            let growthPercent = shouldRender ? ((current.operatingIncome - prior.operatingIncome) / prior.operatingIncome) * 100 : 0

            growth.append(GrowthDataPoint(
                period: current.period,
                growthPercent: growthPercent,
                currentOperatingIncome: current.operatingIncome,
                priorOperatingIncome: prior.operatingIncome,
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("TTM YoY Operating Income Growth")
                                .font(.headline)

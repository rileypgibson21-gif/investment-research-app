//
//  DividendsChartView.swift
//  Ekonix
//
//  Created for dividends quarterly chart
//

import SwiftUI

struct DividendsChartView: View {
    let symbol: String
    let apiService: StockAPIService
    var onDataLoaded: (([DividendsDataPoint]) -> Void)? = nil

    @State private var dividendsData: [DividendsDataPoint] = []
    @State private var isLoading = false
    @State private var selectedBar: UUID?

    var displayData: [DividendsDataPoint] {
        // Sort by period (oldest first on left, newest on right) and take latest 40 quarters
        let sorted = dividendsData.sorted { $0.period < $1.period }
        return Array(sorted.suffix(ChartConstants.quarterlyDataLimit))
    }

    var dividendsRange: (min: Double, max: Double) {
        let values = displayData.map { $0.dividends }
    ...
    var body: some View {
        ZStack {
            ...
            } else if dividendsData.isEmpty {
                VStack(spacing: 20) {
                    Spacer()
                    Image(systemName: "chart.bar.xaxis")
                        .font(.system(size: 60))
                        .foregroundStyle(.gray)
                    Text("No dividends data available")
                                        ...
                                        ZStack(alignment: .bottom) {
                                            ...
                                            HStack(alignment: .bottom, spacing: ChartConstants.barSpacing) {
                                                ForEach(Array(displayData.enumerated()), id: \.element.id) { index, point in
                                                    let heightValue = barHeight(for: point.dividends, in: ChartConstants.chartHeight)
                                                    let offsetValue = barOffset(for: point.dividends, barHeight: heightValue, in: ChartConstants.chartHeight)
                                                        ...
                                                        if selectedBar == point.id {
                                                            VStack(spacing: 2) {
                                                                Text(formatDate(point.period))
                                                                    .font(.caption2)
                                                                    .fontWeight(.bold)
                                                                Text(formatDetailedValue(point.dividends))
                                                                    .font(.caption2)
                                                                    .fontWeight(.semibold)
                                                                #if DEBUG
                                                                Text(point.period)
                                            ...
                                            if dividendsRange.min < 0 && dividendsRange.max > 0 {
                                                Rectangle()
    ...
    private func loadDividends() {
        ...
        Task {
            do {
                let data = try await apiService.fetchDividends(symbol: symbol)
                await MainActor.run {
                    dividendsData = data
                    isLoading = false
                    onDataLoaded?(data)

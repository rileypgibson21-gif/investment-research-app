//
//  EBITDAChartView.swift
//  Ekonix
//
//  Created for ebitda quarterly chart
//

import SwiftUI

struct EBITDAChartView: View {
    let symbol: String
    ...
    private func loadEBITDA() {
        ...
        Task {
            do {
                let data = try await apiService.fetchEBITDA(symbol: symbol)
                await MainActor.run {
                    ebitdaData = data
                    isLoading = false
                    onDataLoaded?(data)

//
//  OperatingMarginChartView.swift
//  Ekonix
//
//  Created for operatingMargin quarterly chart
//

import SwiftUI

struct OperatingMarginChartView: View {
    let symbol: String
    ...
    var displayData: [OperatingMarginDataPoint] {
        ...
        let sorted = operatingMarginData.sorted { $0.period < $1.period }
        return Array(sorted.suffix(ChartConstants.ttmDataLimit))
    }

    var operatingMarginRange: (min: Double, max: Double) {
        let values = displayData.map { $0.operatingMargin }
        return ChartUtilities.calculateAdaptiveRange(values: values)
    }

    private func getYAxisLabels() -> [Double] {
        let range = operatingMarginRange
    ...
    var body: some View {
        ZStack {
            ...
            } else if operatingMarginData.isEmpty {
                VStack(spacing: 20) {
                    Spacer()
                    Image(systemName: "chart.bar.xaxis")
                        .font(.system(size: 60))
                        .foregroundStyle(.gray)
                    Text("No operating margin data available")
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("Trailing Twelve Months Operating Margin")
                                .font(.headline)
                                        ...
                                        ZStack(alignment: .bottom) {
                                            ...
                                            HStack(alignment: .bottom, spacing: ChartConstants.barSpacing) {
                                                ForEach(Array(displayData.enumerated()), id: \.element.id) { index, point in
                                                    let heightValue = barHeight(for: point.operatingMargin, in: ChartConstants.chartHeight)
                                                    let offsetValue = barOffset(for: point.operatingMargin, barHeight: heightValue, in: ChartConstants.chartHeight)
                                                        ...
                                                        if selectedBar == point.id {
                                                            VStack(spacing: 2) {
                                                                Text(formatDate(point.period))
                                                                    .font(.caption2)
                                                                    .fontWeight(.bold)
                                                                Text(formatDetailedValue(point.operatingMargin))
                                                                    .font(.caption2)
                                                                    .fontWeight(.semibold)
                                                                #if DEBUG
                                                                Text(point.period)
                                            ...
                                            if operatingMarginRange.min < 0 && operatingMarginRange.max > 0 {
                                                Rectangle()

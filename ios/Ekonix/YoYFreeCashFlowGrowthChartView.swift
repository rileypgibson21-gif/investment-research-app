//
//  YoYFreeCashFlowGrowthChartView.swift
//  Ekonix
//
//  Created for freeCashFlow YoY growth chart
//

import SwiftUI

struct YoYFreeCashFlowGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let currentFreeCashFlow: Double
        let priorFreeCashFlow: Double
        let shouldRender: Bool
        var id: String { period }
    }

    var growthData: [GrowthDataPoint] {
        guard freeCashFlowData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = freeCashFlowData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.freeCashFlow > 0
            let growthPercent = shouldRender ? ((current.freeCashFlow - prior.freeCashFlow) / prior.freeCashFlow) * 100 : 0

            growth.append(GrowthDataPoint(
                period: current.period,
                growthPercent: growthPercent,
                currentFreeCashFlow: current.freeCashFlow,
                priorFreeCashFlow: prior.freeCashFlow,
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("TTM YoY Free Cash Flow Growth")
                                .font(.headline)

//
//  YoYDividendsGrowthChartView.swift
//  Ekonix
//
//  Created for dividends YoY growth chart
//

import SwiftUI

struct YoYDividendsGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let currentDividends: Double
        let priorDividends: Double
        let shouldRender: Bool
        var id: String { period }
    }

    var growthData: [GrowthDataPoint] {
        guard dividendsData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = dividendsData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.dividends > 0
            let growthPercent = shouldRender ? ((current.dividends - prior.dividends) / prior.dividends) * 100 : 0

            growth.append(GrowthDataPoint(
                period: current.period,
                growthPercent: growthPercent,
                currentDividends: current.dividends,
                priorDividends: prior.dividends,
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("Quarterly YoY Dividends Growth")
                                .font(.headline)

//
//  YoYNetMarginGrowthChartView.swift
//  Ekonix
//
...
struct YoYNetMarginGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let priorNetMargin: Double
        let shouldRender: Bool
        var id: String { period }  // Use period string as stable id
    }

    var growthData: [GrowthDataPoint] {
        guard netMarginData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = netMarginData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.netMargin > 0
            let growthPercent = shouldRender ? ((current.netMargin - prior.netMargin) / prior.netMargin) * 100 : 0

            growth.append(GrowthDataPoint(
                period: current.period,
                growthPercent: growthPercent,
                currentNetMargin: current.netMargin,
                priorNetMargin: prior.netMargin,
        ...
        ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("TTM YoY Net Margin Growth")
                                .font(.headline)

//
//  YoYEBITDAGrowthChartView.swift
//  Ekonix
//
//  Created for ebitda YoY growth chart
//

import SwiftUI

struct YoYEBITDAGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let currentEBITDA: Double
        let priorEBITDA: Double
        let shouldRender: Bool
        var id: String { period }
    }

    var growthData: [GrowthDataPoint] {
        guard ebitdaData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = ebitdaData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.ebitda > 0
            let growthPercent = shouldRender ? ((current.ebitda - prior.ebitda) / prior.ebitda) * 100 : 0

            growth.append(GrowthDataPoint(
                period: current.period,
                growthPercent: growthPercent,
                currentEBITDA: current.ebitda,
                priorEBITDA: prior.ebitda,

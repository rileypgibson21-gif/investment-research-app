//
//  YoYGrossMarginGrowthChartView.swift
//  Ekonix
//
...
struct YoYGrossMarginGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let currentGrossMargin: Double
        let priorGrossMargin: Double
        let shouldRender: Bool
        var id: String { period }  // Use period string as stable id
    }

    var growthData: [GrowthDataPoint] {
        guard grossMarginData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = grossMarginData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.grossMargin > 0
            let growthPercent = shouldRender ? ((current.grossMargin - prior.grossMargin) / prior.grossMargin) * 100 : 0

            growth.append(GrowthDataPoint(
                period: current.period,
                growthPercent: growthPercent,
                currentGrossMargin: current.grossMargin,
                priorGrossMargin: prior.grossMargin,
        ...
        ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("TTM YoY Gross Margin Growth")
                                .font(.headline)

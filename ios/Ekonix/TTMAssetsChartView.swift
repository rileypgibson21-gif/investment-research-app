//
//  TTMAssetsChartView.swift
//  Ekonix
//
//  Created for assets TTM chart
//

import SwiftUI

struct TTMAssetsChartView: View {
    let symbol: String
        ...
        ZStack {
            if isLoading {
                ...
                VStack(spacing: 20) {
                    ...
                    Text("No TTM assets data available")
                        .foregroundStyle(.secondary)
                    Button("Retry") {
                        loadAssets()
                    }
    ...
    private func loadAssets() {
        ...
        Task {
            do {
                let data = try await apiService.fetchTTMAssets(symbol: symbol)
                await MainActor.run {
                    assetsData = data
                    onDataLoaded?(data)
                    isLoading = false
                ...
                await MainActor.run {
                    assetsData = []
                    errorMessage = "Failed to load TTM assets data"
                    onDataLoaded?([])
                    isLoading = false
                }
            }

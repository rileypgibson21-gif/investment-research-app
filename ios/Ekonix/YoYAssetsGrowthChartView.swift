//
//  YoYAssetsGrowthChartView.swift
//  Ekonix
//
//  Created for assets YoY growth chart
//

import SwiftUI

struct YoYAssetsGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let priorAssets: Double
        let shouldRender: Bool
        var id: String { period }
    }

    var growthData: [GrowthDataPoint] {
        guard assetsData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = assetsData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.assets > 0
            let growthPercent = shouldRender ? ((current.assets - prior.assets) / prior.assets) * 100 : 0
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("Quarterly YoY Assets Growth")
                                .font(.headline)

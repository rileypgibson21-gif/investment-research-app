//
//  LiabilitiesChartView.swift
//  Ekonix
//
//  Created for liabilities quarterly chart
//

import SwiftUI

struct LiabilitiesChartView: View {
    let symbol: String
    let apiService: StockAPIService
    var onDataLoaded: (([LiabilitiesDataPoint]) -> Void)? = nil

    @State private var liabilitiesData: [LiabilitiesDataPoint] = []
    @State private var isLoading = false
    @State private var selectedBar: UUID?

    var displayData: [LiabilitiesDataPoint] {
        // Sort by period (oldest first on left, newest on right) and take latest 40 quarters
        let sorted = liabilitiesData.sorted { $0.period < $1.period }
        return Array(sorted.suffix(ChartConstants.quarterlyDataLimit))
    }

    var liabilitiesRange: (min: Double, max: Double) {
        let values = displayData.map { $0.liabilities }
    ...
    var body: some View {
        ZStack {
            ...
            } else if liabilitiesData.isEmpty {
                VStack(spacing: 20) {
                    Spacer()
                    Image(systemName: "chart.bar.xaxis")
                        .font(.system(size: 60))
                        .foregroundStyle(.gray)
                    Text("No liabilities data available")
                                        ...
                                        ZStack(alignment: .bottom) {
                                            ...
                                            HStack(alignment: .bottom, spacing: ChartConstants.barSpacing) {
                                                ForEach(Array(displayData.enumerated()), id: \.element.id) { index, point in
                                                    let heightValue = barHeight(for: point.liabilities, in: ChartConstants.chartHeight)
                                                    let offsetValue = barOffset(for: point.liabilities, barHeight: heightValue, in: ChartConstants.chartHeight)
                                                        ...
                                                        if selectedBar == point.id {
                                                            VStack(spacing: 2) {
                                                                Text(formatDate(point.period))
                                                                    .font(.caption2)
                                                                    .fontWeight(.bold)
                                                                Text(formatDetailedValue(point.liabilities))
                                                                    .font(.caption2)
                                                                    .fontWeight(.semibold)
                                                                #if DEBUG
                                                                Text(point.period)
                                            ...
                                            if liabilitiesRange.min < 0 && liabilitiesRange.max > 0 {
                                                Rectangle()
    ...
    private func loadLiabilities() {
        ...
        Task {
            do {
                let data = try await apiService.fetchLiabilities(symbol: symbol)
                await MainActor.run {

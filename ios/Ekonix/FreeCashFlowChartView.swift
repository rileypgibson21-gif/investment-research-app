//
//  FreeCashFlowChartView.swift
//  Ekonix
//
//  Created for freeCashFlow quarterly chart
//

import SwiftUI

struct FreeCashFlowChartView: View {
    let symbol: String
    let apiService: StockAPIService
    var onDataLoaded: (([FreeCashFlowDataPoint]) -> Void)? = nil
    ...
    var displayData: [FreeCashFlowDataPoint] {
        ...
        let sorted = freeCashFlowData.sorted { $0.period < $1.period }
        return Array(sorted.suffix(ChartConstants.quarterlyDataLimit))
    }

    var freeCashFlowRange: (min: Double, max: Double) {
        let values = displayData.map { $0.freeCashFlow }
    ...
    var body: some View {
        ZStack {
            ...
            } else if freeCashFlowData.isEmpty {
                VStack(spacing: 20) {
                    Spacer()
                    Image(systemName: "chart.bar.xaxis")
                        .font(.system(size: 60))
                        .foregroundStyle(.gray)
                    Text("No freeCashFlow data available")
                                        ...
                                        ZStack(alignment: .bottom) {
                                            ...
                                            HStack(alignment: .bottom, spacing: ChartConstants.barSpacing) {
                                                ForEach(Array(displayData.enumerated()), id: \.element.id) { index, point in
                                                    let heightValue = barHeight(for: point.freeCashFlow, in: ChartConstants.chartHeight)
                                                    let offsetValue = barOffset(for: point.freeCashFlow, barHeight: heightValue, in: ChartConstants.chartHeight)
                                                        ...
                                                        if selectedBar == point.id {
                                                            VStack(spacing: 2) {
                                                                Text(formatDate(point.period))
                                                                    .font(.caption2)
                                                                    .fontWeight(.bold)
                                                                Text(formatDetailedValue(point.freeCashFlow))
                                                                    .font(.caption2)
                                                                    .fontWeight(.semibold)
                                                                #if DEBUG
                                                                Text(point.period)
                                            ...
                                            if freeCashFlowRange.min < 0 && freeCashFlowRange.max > 0 {
                                                Rectangle()
    ...
    private func loadFreeCashFlow() {
        isLoading = true

        Task {
            do {
                let data = try await apiService.fetchFreeCashFlow(symbol: symbol)
                await MainActor.run {
                    freeCashFlowData = data
                    isLoading = false
                    onDataLoaded?(data)
                }
            } catch {

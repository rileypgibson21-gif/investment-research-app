//
//  YoYLiabilitiesGrowthChartView.swift
//  Ekonix
//
//  Created for liabilities YoY growth chart
//

import SwiftUI

struct YoYLiabilitiesGrowthChartView: View {
    let symbol: String
    ...
    struct GrowthDataPoint: Identifiable {
        ...
        let priorLiabilities: Double
        let shouldRender: Bool
        var id: String { period }
    }

    var growthData: [GrowthDataPoint] {
        guard liabilitiesData.count >= 5 else { return [] }

        var growth: [GrowthDataPoint] = []
        let sortedData = liabilitiesData.sorted { $0.period < $1.period }
        ...
        for i in 4..<sortedData.count {
            ...
            let shouldRender = prior.liabilities > 0
            let growthPercent = shouldRender ? ((current.liabilities - prior.liabilities) / prior.liabilities) * 100 : 0

            growth.append(GrowthDataPoint(
                period: current.period,
                growthPercent: growthPercent,
                currentLiabilities: current.liabilities,
                priorLiabilities: prior.liabilities,
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            Text("Quarterly YoY Liabilities Growth")
                                .font(.headline)
